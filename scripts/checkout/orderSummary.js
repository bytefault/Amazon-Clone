...
//renderOrderSummary(); ab yaha run nhi karna , checkout.js me karo
...
/* 
  ...
  the technique we just use where we update the data and then regenerate all the html is called MVC (Model - View - Controller).
  In mvc we split our code into 3 parts and then they interact with each other in a loop
  1. Model = saves and manages the data (like our data folder is Model)
  2. View = takes the data and display it on the page (like checkout.js)
  3. Controller = runs some code when we interact with the page (like event listeners)
  Here we took the model or the data which is our cart and then we generate the HTML or the view then when we interact with the view it will run the controller, then controller will update the model and finally we use the updated model to regenerate the view like we run render fn inside render fn.
  */
